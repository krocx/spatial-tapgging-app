...
final class SIBClient {
    ...
    func duplicateAnchor(id: String, assetId: String? = nil) async throws -> Anchor {
        struct Body: Encodable { let assetId: String?; let createdBy: String? }
        return try await post(Anchor.self, path: "/anchors/\(id)/duplicate",
                              body: Body(assetId: assetId,
    ...
    /// U2: copy a Guide onto another anchor (steps, media, model assignments
    /// and flags travel; pins, placement, training and sharing don't). The
    /// copy is a draft on the target anchor.
    func copyGuide(id: String, toAnchor anchorId: String, name: String? = nil) async throws -> ARGuide {
        struct Body: Encodable { let anchorId: String; let name: String?; let createdBy: String? }
        return try await post(ARGuide.self, path: "/guides/\(id)/copy",
                              body: Body(anchorId: anchorId, name: name,
                                         createdBy: settings.authorName.isEmpty ? nil : settings.authorName))
    }
