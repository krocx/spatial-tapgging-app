...
export function LaneLayer(): JSX.Element | null {
  ...
  // Night: bands become faint light washes on the dark canvas; separators and
      ...
      {columns.map((lane, i) => (
        <g key={lane.id}>
          <rect
            ...
            fill={i % 2 === 0 ? (bandA ?? 'rgba(47,111,237,0.045)') : (bandB ?? 'rgba(100,116,139,0.045)')}
          />
          <line x1={lane.x} y1={BAND_TOP} x2={lane.x} y2={BAND_TOP + BAND_H}
                stroke={sepColor} strokeWidth={1.5 / camera.scale} />
          {i === columns.length - 1 && (
      ...
      {rows.map((lane, i) => (
        <g key={lane.id}>
          <rect
            ...
            fill={i % 2 === 0 ? (bandA ?? 'rgba(22,163,74,0.04)') : (bandB ?? 'rgba(100,116,139,0.04)')}
          />
