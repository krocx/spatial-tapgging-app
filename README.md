...
- **Procedure Designer** - procedures can be authored visually on the Roadmap canvas as `procedure` maps: nodes are steps, coloured edges are branch logic (Next / On failure / Requires), and step numbers are derived from the graph by the server. A validated map compiles into a draft guide in the Guide Library, with every step unplaced - AR placement stays on device, and re-syncing an edited map never overwrites placement. See **[docs/PROCEDURE-DESIGNER.md](docs/PROCEDURE-DESIGNER.md)**.

### 3D Model Library
A shared 3D asset library with per-anchor "kits": models can be assigned to specific anchors or marked `general` to be visible everywhere, with an Author-saved default scale. GLB/USDZ upload is pass-through; OBJ/FBX/STEP are converted asynchronously to GLB via headless Blender when it is available on the host. GLB→USDZ conversion runs **in the portal browser** (Three.js `USDZExporter`) and uploads the result back, so no native toolchain is required on the server. Models render on device as ghost overlays with per-step scale, opacity, offset, and Y-rotation, positioned in AR by the Author. The device-facing API is read-only: `GET /models?anchorId=` lists an anchor's kit **plus all `general` models**, then the app downloads each selected model via `GET /models/:id/file.usdz` (preferred; GLB fallback at `/file.glb`) and caches it locally - uploads, conversion write-back, kit assignment, and deletes are portal-only.
...
A secure, collaborative mind-mapping and roadmapping tool hosted on SIB - built for roadmap design, SIB ontology graphs, and AR workflow planning. Highlights: real-time collaboration (WebSockets, live cursors, presence), swimlanes (Now/Next/Later columns, Why/What/How rows), node status + review verdicts + comments, milestones, icons/shapes/links, collapsible branches, presentation mode, view filters and custom groups, version history with restore, PNG/SVG/JSON export, cross-server JSON import, a draft→publish workflow (per-map draft keys, pre-RBAC), SIB ontology import/export, and **image import** - photograph a whiteboard and a local vision model (Ollama) turns it into an editable roadmap draft. The same canvas also powers the **Procedure Designer**: maps created with `kind: 'procedure'` author AR work instructions visually and compile straight into the Guide Library (see [docs/PROCEDURE-DESIGNER.md](docs/PROCEDURE-DESIGNER.md)). Full docs: [docs/roadmap-mindmapper.md](docs/roadmap-mindmapper.md). The platform's own roadmap lives in it: [docs/roadmaps/](docs/roadmaps/).
...
| See every platform capability at a glance (the canonical feature index) | **[docs/FEATURE-CATALOG.md](docs/FEATURE-CATALOG.md)** |
...
| Design AR work instructions visually on the roadmap canvas | **[docs/PROCEDURE-DESIGNER.md](docs/PROCEDURE-DESIGNER.md)** |
| Read what a phone logged (QA Mode, Admin → Device Logs) | [docs/QA-LOGGING.md](docs/QA-LOGGING.md) |
| Icon library (every icon in the Designer, generated from code) | [docs/ICONOGRAPHY.md](docs/ICONOGRAPHY.md) · [docs/iconography.html](docs/iconography.html) - `npm run icons:doc` |
