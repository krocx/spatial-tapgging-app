...
*Setup (one-time, per SIB host — never on the user's device):* point `SIB_VISION_URL` at any OpenAI-compatible endpoint that accepts `image_url` content. Options: Ollama on the SIB host (`ollama pull qwen2.5vl`, URL `http://localhost:11434/v1`), vLLM / LM Studio on a GPU box, or a company LLM gateway. If `SIB_VISION_URL` is unset SIB falls back to `ASK_LLM_URL` (the Ask SIB gateway) when that is multimodal; if neither is set the home page shows the *From a photo* door as "Not set up on this server" (`GET /mindmap/import-image/status`) instead of timing out. Optional: `SIB_VISION_MODEL`, `SIB_VISION_API_KEY`, `SIB_VISION_TIMEOUT_MS` (default 120 s). Endpoint: `POST /mindmap/import-image` `{ image: base64, mimeType }` → preview graph (never auto-saved); the preview lets you create the draft as a **Roadmap** or a **Procedure** (arrows → *Next*). The photo goes only to the configured endpoint — a cloud gateway means the whiteboard leaves the site.
