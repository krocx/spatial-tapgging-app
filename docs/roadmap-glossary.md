...
- **Assembly Model** ✅ — one 3D model of the whole assembly whose named nodes are the parts; guide steps address parts by node name (show, ghost, insert, highlight) instead of placing a model per step. The registration of the base part is the PartFrame (AR OJT).
...
- **Focus Area** ✅ — a numbered audit group from the Audit Reference Library (e.g. *14 — 6S Audits*). Auditors pick a Focus Area, then one of its pre-defined questions, so findings are logged in Corporate Quality's vocabulary instead of free text. Maintained in the portal; importable from the same Excel the SharePoint lists were kept in.
- **Finding Category** ✅ — the disposition of a Gemba finding: **Strength**, **OFI** (Opportunity for Improvement) or **NC** (Non-Conformance), with an optional preliminary risk rating 0–3.
